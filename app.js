...
function createGuessYouLike(data) {
  ...
  switch (n) {
    case 1: 
      ...
      break
    case 2:
      data['result'] = data.id % 2 === 0 ? '《Improving Psychiatric Care》' : '《模拟2-none》'
      break
    case 3:
      data['result'] = '《模拟3》'
...
// 创建categoryList数据
function createCategoryDate(data) {
...
app.get('/book/home', (rep, res) => {
  ...
  conn.query('select * from book where cover !=\'\'', (err, results) => {
    ...
    const featured = []
    const random = []
    const categoryList = createCategoryDate(results)
    const categories = [
      {
        category: 1,
        num: 56,
